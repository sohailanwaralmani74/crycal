(function() {
  ...
  function calculateScore(inputs) {
    ...
    var frq = Math.min(18, Math.max(0, inputs.frq));

    var composite = Math.round((mcq / 55) * 45 + (frq / 18) * 55);
    var apGrade = 1;
    var statusText = 'No Credit (Grade 1)';
    if (composite >= 74) { apGrade = 5; statusText = 'Qualified for Credit (Grade 5 - Extremely Well Qualified)'; }
    else if (composite >= 62) { apGrade = 4; statusText = 'Qualified for Credit (Grade 4 - Well Qualified)'; }
    else if (composite >= 50) { apGrade = 3; statusText = 'Qualified for Credit (Grade 3 - Qualified)'; }
    else if (composite >= 37) { apGrade = 2; statusText = 'No Credit (Grade 2 - Possibly Qualified)'; }
