(function() {
  ...
  function calculateResults(inputs) {
    ...
    var loanAmt = Math.max(0, hp - dp);

    var r = (inputs.interestRate / 100) / 12;
    var n = inputs.loanTermYears * 12;

    if (loanAmt <= 0 || r <= 0 || n <= 0) return null;

    var monthlyPmt = (loanAmt * r * Math.pow(1 + r, n)) / (Math.pow(1 + r, n) - 1);
    var totalInterest = (monthlyPmt * n) - loanAmt;
  ...
  function updateTool() {
    ...
    setOutputText('output_downPaymentAmount', formatCurrencyLocal(result.dp));
    setOutputText('output_jumboLoanAmount', formatCurrencyLocal(result.loanAmt));
    ...
    if (typeof window.logHistory === 'function') {
      window.logHistory({
        ...
        jumboLoanAmount: result.loanAmt,
  ...
  function generateChartData(tab, result, inputs) {
    ...
    if (tab === 'breakdown') {
      return {
        type: 'doughnut',
        data: {
          labels: ['Down Payment', 'Jumbo Loan Principal', 'Total Interest'],
          datasets: [{
            data: [result.dp, result.loanAmt, result.totalInterest],
        ...
        options: {
          ...
          plugins: {
            ...
            title: { display: true, text: 'Total Purchase Financing Allocation' }
          },
          cutout: '60%'
        }
      };
    ...
    if (tab === 'amortization') {
      ...
      return {
        ...
        data: {
          labels: labels,
          datasets: [{
            label: 'Remaining Jumbo Loan Balance',
        ...
        options: {
          ...
          plugins: {
            legend: { position: 'top' },
            title: { display: true, text: 'Amortization Payoff Schedule' }
          },
          scales: {
            y: {
              beginAtZero: true,
