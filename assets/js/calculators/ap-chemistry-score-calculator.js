(function() {
  ...
  function calculateScore(inputs) {
    ...
    var frq = Math.min(46, Math.max(0, inputs.frq));

    var composite = Math.round((mcq / 60) * 50 + (frq / 46) * 50);
    var apGrade = 1;
    var statusText = 'No Credit (Grade 1)';
    if (composite >= 79) { apGrade = 5; statusText = 'Qualified for Credit (Grade 5 - Extremely Well Qualified)'; }
    else if (composite >= 63) { apGrade = 4; statusText = 'Qualified for Credit (Grade 4 - Well Qualified)'; }
    else if (composite >= 47) { apGrade = 3; statusText = 'Qualified for Credit (Grade 3 - Qualified)'; }
    else if (composite >= 32) { apGrade = 2; statusText = 'No Credit (Grade 2 - Possibly Qualified)'; }
