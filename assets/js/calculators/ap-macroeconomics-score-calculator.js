(function() {
  ...
  function calculateScore(inputs) {
    ...
    var frq = Math.min(20, Math.max(0, inputs.frq));

    var composite = Math.round((mcq / 60) * 60 + (frq / 20) * 30);
    var apGrade = 1;
    ...
    else if (composite >= 55) { apGrade = 4; statusText = 'Qualified for Credit (Grade 4 - Well Qualified)'; }
    else if (composite >= 42) { apGrade = 3; statusText = 'Qualified for Credit (Grade 3 - Qualified)'; }
    else if (composite >= 30) { apGrade = 2; statusText = 'No Credit (Grade 2 - Possibly Qualified)'; }
  ...
  function updateTool() {
    ...
    setOutputText('output_compositeScore', res.compositeScore.toString() + ' / 90');
