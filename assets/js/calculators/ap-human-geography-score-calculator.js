(function() {
  ...
  function calculateScore(inputs) {
    var mcq = Math.min(60, Math.max(0, inputs.mcq));
    var frq = Math.min(21, Math.max(0, inputs.frq));

    var composite = Math.round((mcq / 60) * 60 + (frq / 21) * 60);
    var apGrade = 1;
    var statusText = 'No Credit (Grade 1)';
    if (composite >= 88) { apGrade = 5; statusText = 'Qualified for Credit (Grade 5 - Extremely Well Qualified)'; }
    else if (composite >= 73) { apGrade = 4; statusText = 'Qualified for Credit (Grade 4 - Well Qualified)'; }
    else if (composite >= 58) { apGrade = 3; statusText = 'Qualified for Credit (Grade 3 - Qualified)'; }
    else if (composite >= 43) { apGrade = 2; statusText = 'No Credit (Grade 2 - Possibly Qualified)'; }
  ...
  function updateTool() {
    ...
    setOutputText('output_compositeScore', res.compositeScore.toString() + ' / 120');
