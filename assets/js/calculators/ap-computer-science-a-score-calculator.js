(function() {
  ...
  function calculateScore(inputs) {
    var mcq = Math.min(40, Math.max(0, inputs.mcq));
    var frq = Math.min(36, Math.max(0, inputs.frq));

    var composite = Math.round((mcq / 40) * 40 + (frq / 36) * 40);
    var apGrade = 1;
    var statusText = 'No Credit (Grade 1)';
    if (composite >= 62) { apGrade = 5; statusText = 'Qualified for Credit (Grade 5 - Extremely Well Qualified)'; }
    else if (composite >= 49) { apGrade = 4; statusText = 'Qualified for Credit (Grade 4 - Well Qualified)'; }
    else if (composite >= 37) { apGrade = 3; statusText = 'Qualified for Credit (Grade 3 - Qualified)'; }
    else if (composite >= 26) { apGrade = 2; statusText = 'No Credit (Grade 2 - Possibly Qualified)'; }
