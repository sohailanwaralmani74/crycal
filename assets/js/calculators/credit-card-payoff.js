...
(function() {
  ...
  function getMonthlyRate(annualRate, frequency) {
    var apr = annualRate / 100;
    if (frequency === 'daily') {
      var dailyRate = apr / 365;
      return Math.pow(1 + dailyRate, 30) - 1; // approximate monthly
    } else {
      return apr / 12;
    }
  }
  ...
  // ── Simulate Payoff ──
  function simulatePayoff(balance, annualRate, monthlyPayment, extraPayment, freq) {
    var monthlyRate = getMonthlyRate(annualRate, freq);
  ...
  function calculatePayoff(inputs) {
    var freq = inputs.compoundingFrequency || 'daily';
    ...
    if (tab === 'comparison') {
      ...
      return {
        ...
        options: {
          ...
          plugins: {
            legend: { position: 'top' },
            title: { display: true, text: 'Credit Card Balance Over Time', font: { size: 14 } }
          },
          scales: {
            y: {
              beginAtZero: true,
