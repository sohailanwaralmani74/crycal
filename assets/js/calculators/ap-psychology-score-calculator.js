(function() {
  ...
  function calculateScore(inputs) {
    var mcq = Math.min(75, Math.max(0, inputs.mcq));
    var frq = Math.min(14, Math.max(0, inputs.frq));

    var composite = Math.round((mcq / 75) * 100 + (frq / 14) * 50);
    var apGrade = 1;
    var statusText = 'No Credit (Grade 1)';
    if (composite >= 110) { apGrade = 5; statusText = 'Qualified for Credit (Grade 5 - Extremely Well Qualified)'; }
    else if (composite >= 92) { apGrade = 4; statusText = 'Qualified for Credit (Grade 4 - Well Qualified)'; }
    else if (composite >= 77) { apGrade = 3; statusText = 'Qualified for Credit (Grade 3 - Qualified)'; }
    else if (composite >= 62) { apGrade = 2; statusText = 'No Credit (Grade 2 - Possibly Qualified)'; }
  ...
  function updateTool() {
    ...
    setOutputText('output_compositeScore', res.compositeScore.toString() + ' / 150');
    ...
    return {
      ...
      data: {
        ...
        datasets: [{
          ...
          data: [61, 76, 91, 109, 150, userComp],
          backgroundColor: [
            '#ef4444',
            '#f97316',
            '#eab308',
