(function() {
  ...
  function calculateScore(inputs) {
    ...
    var frq = Math.min(17, Math.max(0, inputs.frq));

    var composite = Math.round((mcq / 55) * 60 + (frq / 17) * 60);
    var apGrade = 1;
    var statusText = 'No Credit (Grade 1)';
    if (composite >= 85) { apGrade = 5; statusText = 'Qualified for Credit (Grade 5 - Extremely Well Qualified)'; }
    else if (composite >= 70) { apGrade = 4; statusText = 'Qualified for Credit (Grade 4 - Well Qualified)'; }
    else if (composite >= 54) { apGrade = 3; statusText = 'Qualified for Credit (Grade 3 - Qualified)'; }
    else if (composite >= 40) { apGrade = 2; statusText = 'No Credit (Grade 2 - Possibly Qualified)'; }
