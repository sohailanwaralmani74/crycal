...
(function() {
    ...
    if (tab === 'comparison') {
      ...
      return {
        ...
        options: {
          ...
          plugins: {
            legend: { position: 'top' },
            title: { display: true, text: 'Student Loan Balance Over Time', font: { size: 14 } }
          },
          scales: {
            y: {
              beginAtZero: true,
  ...
  function resetTool() {
    ...
    var _el_input_monthlyPayment = document.getElementById('input_monthlyPayment');
    _el_input_monthlyPayment.value = (_el_input_monthlyPayment.dataset && _el_input_monthlyPayment.dataset.default !== undefined) ? _el_input_monthlyPayment.dataset.default : (_el_input_monthlyPayment.getAttribute('value') || '');
