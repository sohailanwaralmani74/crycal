...
(function() {
  ...
  // ── Simulate debt payoff (snowball method) ──
  function simulateSnowball(debts, extraPayment, freq) {
    ...
    while (month < maxMonths) {
      ...
      // Apply payment to debts in order (smallest balance first)
      active.forEach(function(d) {
        ...
        // If this is the first active debt, apply any extra to it
  ...
  function updateDebtsFromUI() {
    ...
    items.forEach(function(item) {
      ...
      var rate = parseFloat(item.querySelector('.debt-rate').value) || 0;
      var min = parseFloat(item.querySelector('.debt-min').value) || 0;
      // Preserve the original debt id
      var existing = debts.find(function(d) { return d.id === id; });
      newDebts.push({
        id: id,
        name: name,
        balance: balance,
  ...
  function updateTool() {
    ...
    var result = simulateSnowball(validDebts, inputs.extraMonthlyPayment, inputs.compoundingFrequency);
  ...
  function generateChartData(tab, result, inputs) {
    ...
    if (tab === 'progress') {
      ...
      return {
        ...
        options: {
          ...
          plugins: {
            legend: { display: false },
            title: { display: true, text: 'Debt Snowball Progress', font: { size: 14 } }
          },
          scales: {
            y: {
              beginAtZero: true,
    ...
    if (tab === 'timeline') {
      ...
      var sorted = debtData2.slice().sort(function(a, b) { return a.balance - b.balance; });
      ...
      sorted.forEach(function(d) {
        ...
        payoffLabels.push(d.name);
        payoffMonths.push(found ? found.paidOffMonth : '-');
      });

      // Create a bar chart showing when each debt is paid off
      ...
      return {
        ...
        options: {
          ...
          plugins: {
            legend: { display: false },
            title: { display: true, text: 'Debt Payoff Timeline', font: { size: 14 } },
            tooltip: {
              callbacks: {
                label: function(ctx) {
