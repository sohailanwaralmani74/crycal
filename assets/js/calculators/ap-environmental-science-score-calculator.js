(function() {
  ...
  function calculateScore(inputs) {
    var mcq = Math.min(80, Math.max(0, inputs.mcq));
    var frq = Math.min(30, Math.max(0, inputs.frq));

    var composite = Math.round((mcq / 80) * 90 + (frq / 30) * 60);
    var apGrade = 1;
    var statusText = 'No Credit (Grade 1)';
    if (composite >= 105) { apGrade = 5; statusText = 'Qualified for Credit (Grade 5 - Extremely Well Qualified)'; }
    else if (composite >= 88) { apGrade = 4; statusText = 'Qualified for Credit (Grade 4 - Well Qualified)'; }
    else if (composite >= 71) { apGrade = 3; statusText = 'Qualified for Credit (Grade 3 - Qualified)'; }
    else if (composite >= 54) { apGrade = 2; statusText = 'No Credit (Grade 2 - Possibly Qualified)'; }
    ...
    return {
      ...
      data: {
        ...
        datasets: [{
          ...
          data: [53, 70, 87, 104, 150, userComp],
          backgroundColor: [
            '#ef4444',
            '#f97316',
            '#eab308',
