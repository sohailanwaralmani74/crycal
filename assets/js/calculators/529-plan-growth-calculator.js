(function() {
  ...
  function getInputs() {
    ...
    var yearsUntilCollege = parseFloat(document.getElementById('input_yearsUntilCollege').value) || 0;
    var stateTaxDeductionRate = parseFloat(document.getElementById('input_stateTaxDeductionRate').value) || 0;
  ...
  var ALL_OUTPUTS = ['projectedBalance', 'totalContributions', 'totalGrowth', 'annualTaxSavings', 'totalTaxSavings'];

  function clearOutputs() {
    ALL_OUTPUTS.forEach(function(id) { setOutputText('output_' + id, '—'); });
  }

  function calculate529(inputs) {
    var P = inputs.currentBalance;
    var PMT = inputs.monthlyContribution;
    var r = inputs.annualReturn;
    var t = inputs.yearsUntilCollege;
    var taxRate = inputs.stateTaxDeductionRate;

    if (P <= 0 && PMT <= 0) {
      return { error: 'Enter a current balance or monthly contribution' };
    }
    if (t <= 0 || r < 0) {
    ...
    var annualTaxSavings = annualContribution * taxRate;
    var totalTaxSavings = annualTaxSavings * t;
  ...
  function updateTool() {
    ...
    if (inputs.currentBalance <= 0 && inputs.monthlyContribution <= 0) {
      clearOutputs();
      lastChartData = null;
      updateCharts(null);
      return;
    ...
    var result = calculate529(inputs);
    ...
    setOutputText('output_annualTaxSavings', inputs.stateTaxDeductionRate > 0 ? formatCurrency(result.annualTaxSavings) : '—');
    setOutputText('output_totalTaxSavings', inputs.stateTaxDeductionRate > 0 ? formatCurrency(result.totalTaxSavings) : '—');
  ...
  function generateChartData(tab, data) {
    ...
    if (tab === 'growth') {
      ...
      return {
        ...
        data: {
          labels: fLabels,
          datasets: [
            { label: '529 Balance', data: fBalances, borderColor: '#4A90D9', backgroundColor: 'rgba(74, 144, 217, 0.1)', fill: true, tension: 0.3, pointRadius: 0, borderWidth: 2 },
        ...
        options: {
          ...
          plugins: {
            ...
            title: { display: true, text: '529 Plan Growth', font: { size: 14, color: '#e8edf0' } }
          },
          scales: {
    ...
    if (tab === 'breakdown') {
      ...
      return {
        type: 'doughnut',
        data: {
          labels: ['Contributions', 'Tax-Free Growth'],
